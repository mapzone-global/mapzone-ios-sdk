...
/**
 ...
 Values of this type are used in the `MZGLRasterStyleLayer.rasterResamplingMode`
 property.
 */
...
/**
 An `MZGLRasterStyleLayer` is a style layer that renders georeferenced raster
 imagery on the map, especially raster tiles.
 
 Use a raster style layer to configure the color parameters of raster tiles
 loaded by an `MZGLRasterTileSource` object or raster images loaded by an
 `MZGLImageSource` object. For example, you could use a raster style layer to
 ...
 register an image for use as an icon or pattern, use the `-[MZGLStyle
 setImage:forName:]` method. To configure a point annotation’s image, use the
 `MZGLAnnotationImage` class.
 ...
 ```swift
 let layer = MZGLRasterStyleLayer(identifier: "clouds", source: source)
 layer.rasterOpacity = NSExpression(forConstantValue: 0.5)
 mapView.style?.addLayer(layer)
 ```
 */
MGL_EXPORT
@interface MZGLRasterStyleLayer : MZGLForegroundStyleLayer
...
/**
 ...
 `MZGLRasterStyleLayer` object and unwrapping its raw value.
 */
@interface NSValue (MZGLRasterStyleLayerAdditions)

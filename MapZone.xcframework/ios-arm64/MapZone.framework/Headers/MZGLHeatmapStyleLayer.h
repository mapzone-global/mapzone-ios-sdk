...
/**
 An `MZGLHeatmapStyleLayer` is a style layer that renders a <a
 ...
 features can come from vector tiles loaded by an `MZGLVectorTileSource` object,
 or they can be `MZGLPointAnnotation`, `MGLPointFeature`, `MZGLPointCollection`,
 or `MZGLPointCollectionFeature` instances in an `MZGLShapeSource` or
 `MZGLComputedShapeSource` object.
 
 Consider accompanying a heatmap style layer with an `MZGLCircleStyleLayer` or
 `MZGLSymbolStyleLayer` at high zoom levels. If you are unsure whether the point
 data in an `MZGLShapeSource` is dense enough to warrant a heatmap, you can
 alternatively cluster the source using the `MZGLShapeSourceOptionClustered`
 option and render the data using an `MZGLCircleStyleLayer` or
 `MZGLSymbolStyleLayer`.
 ...
 ```swift
 let layer = MZGLHeatmapStyleLayer(identifier: "earthquake-heat", source: earthquakes)
 ...
 */
MGL_EXPORT
@interface MZGLHeatmapStyleLayer : MZGLVectorStyleLayer

...
*/
MGL_EXPORT
@interface MZGLMapProjection : NSObject
 ...
 */
- (instancetype)initWithMapView:(MZGLMapView*)mapView;

/**
 A camera representing the current projection state
 */
@property (readonly, copy) MZGLMapCamera *camera;
...
/**
 ...
 @note `MZGLMapView` instance frame must not be changed since this projection is initialized,
 	otherwise the calculation may be wrong.
 */
- (void)setCamera:(MZGLMapCamera * _Nonnull)camera withEdgeInsets:(UIEdgeInsets)insets;

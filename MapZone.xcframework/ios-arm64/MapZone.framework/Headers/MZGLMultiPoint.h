...
/**
 The `MZGLMultiPoint` class is an abstract superclass used to define shapes
 composed of multiple vertices.

 Create instances of `MZGLPolyline` or `MZGLPolygon` in order to use 
 properties of `MZGLMultiPoint`. Do not create instances of `MZGLMultiPoint`
 ...
 Do not confuse `MZGLMultiPoint` with `MZGLPointCollection`, which represents a
 collection of related but disconnected points.
 */
MGL_EXPORT
@interface MZGLMultiPoint : MZGLShape
...
/**
 ...
 immediately. If the shape is part of an `MZGLShapeSource` object, you must
 explicitly set the `MZGLShapeSource.shape` property in order for any style

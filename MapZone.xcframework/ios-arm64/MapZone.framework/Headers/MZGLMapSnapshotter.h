...
@protocol MZGLMapSnapshotterDelegate;

/**
 An overlay that is placed within a `MGLMapSnapshot`.
 To access this object, use `-[MZGLMapSnapshotter startWithOverlayHandler:completionHandler:]`.
 */
...
/**
 The options to use when creating images with the `MZGLMapSnapshotter`.
 */
MGL_EXPORT
 ...
 */
- (instancetype)initWithStyleURL:(nullable NSURL *)styleURL camera:(MZGLMapCamera *)camera size:(CGSize)size;
 ...
 */
@property (nonatomic) MZGLMapCamera *camera;
...
/**
 An `MZGLMapSnapshotter` generates static raster images of the map. Each snapshot
 ...
 asynchronously, calling `MZGLMapSnapshotterDelegate` methods if defined, then
 ...
 For an interactive map, use the `MZGLMapView` class. Both `MZGLMapSnapshotter`
 and `MZGLMapView` are compatible with offline packs managed by the
 `MZGLOfflineStorage` class.
 ...
 ```swift
 let camera = MZGLMapCamera(lookingAtCenter: CLLocationCoordinate2D(latitude: 37.7184, longitude: -122.4365), altitude: 100, pitch: 20, heading: 0)
 
 let options = MGLMapSnapshotOptions(styleURL: MZGLStyle.satelliteStreetsStyleURL, camera: camera, size: CGSize(width: 320, height: 480))
 options.zoomLevel = 10
 
 let snapshotter = MZGLMapSnapshotter(options: options)
 ...
 `MZGLMapSnapshotter` to generate a static image based on an `MZGLMapView`
 object's style, camera, and view bounds.
 */
MGL_EXPORT
@interface MZGLMapSnapshotter : NSObject <MGLStylable>
...
/**
 ...
 the snapshot, create a new `MZGLMapSnapshotter` object.
 */
- (void)cancel;
 ...
 */
@property (nonatomic, weak) id <MZGLMapSnapshotterDelegate> delegate;
...
/**
 ...
 `-[MZGLMapSnapshotterDelegate mapSnapshotter:didFinishLoadingStyle:]` method. It
...
/**
 ...
 `MZGLMapSnapshotter` object.
 */
@protocol MZGLMapSnapshotterDelegate <NSObject>
@optional
...
*/
- (void)mapSnapshotterDidFail:(MZGLMapSnapshotter *)snapshotter withError:(NSError *)error;

/**
 Tells the delegate that the snapshotter has just finished loading a style.
 
 This method is called in response to
 `-[MZGLMapSnapshotter startWithQueue:completionHandler:]` as long as the
 `MZGLMapSnapshotter.delegate` property is set. Changes to sources or layers of
 ...
 */
- (void)mapSnapshotter:(MZGLMapSnapshotter *)snapshotter didFinishLoadingStyle:(MZGLStyle *)style;

- (void)mapSnapshotter:(MZGLMapSnapshotter *)snapshotter didFailLoadingImageNamed:(NSString *)name;

...
/**
 ...
 MZGLStyle provides a set of convenience methods for changing Mapbox
 default styles using `MZGLMapView.styleURL`.
 ...
 via `MZGLMapView.style` by updating the style's data sources or layers.

 @note Wait until the map style has finished loading before modifying a map's
    style via any of the `MZGLStyle` instance methods below. You can use the
    `-[MZGLMapViewDelegate mapView:didFinishLoadingStyle:]` or
    `-[MZGLMapViewDelegate mapViewDidFinishLoadingMap:]` methods as indicators
    that it's safe to modify the map's style.

 #### Related examples
 See the <a href="https://docs.mapbox.com/ios/maps/examples/default-styles/">
 Default styles</a> example to learn how to initialize an `MZGLMapView` object
 with a Mapbox default style using `MZGLStyle`'s class methods.
 */
MGL_EXPORT
@interface MZGLStyle : NSObject
 ...
 */
+ (NSArray<MZGLDefaultStyle*>*) predefinedStyles;

/**
 Returns default style
 */
+ (MZGLDefaultStyle*) defaultStyle;
 ...
 */
+ (MZGLDefaultStyle*) predefinedStyle:(NSString*)withStyleName;
 ...
 */
@property (nonatomic, strong) NSSet<__kindof MZGLSource *> *sources;
...
/**
 ...
 @return An instance of a concrete subclass of `MZGLSource` associated with the
    given identifier, or `nil` if the current style contains no such source.
 */
- (nullable MZGLSource *)sourceWithIdentifier:(NSString *)identifier;
 ...
 */
- (void)removeSource:(MZGLSource *)source;
 ...
 */
- (BOOL)removeSource:(MZGLSource *)source error:(NSError * __nullable * __nullable)outError;
 ...
 */
@property (nonatomic, strong) NSArray<__kindof MZGLStyleLayer *> *layers;
...
/**
 ...
 @return An instance of a concrete subclass of `MZGLStyleLayer` associated with
    the given identifier, or `nil` if the current style contains no such style
    layer.
 */
- (nullable MZGLStyleLayer *)layerWithIdentifier:(NSString *)identifier;
...
/**
 ...
 @param layer The layer object to add to the map view. This object must be an
    instance of a concrete subclass of `MZGLStyleLayer`.
 */
- (void)addLayer:(MZGLStyleLayer *)layer;
 ...
 */
- (void)insertLayer:(MZGLStyleLayer *)layer atIndex:(NSUInteger)index;
 ...
 */
- (void)insertLayer:(MZGLStyleLayer *)layer belowLayer:(MZGLStyleLayer *)sibling;
...
/**
 ...
 existing layer.
 */
- (void)insertLayer:(MZGLStyleLayer *)layer aboveLayer:(MZGLStyleLayer *)sibling;
...
/**
 ...
 must conform to the `MZGLStyleLayer` protocol.
 */
- (void)removeLayer:(MZGLStyleLayer *)layer;
...
/**
 ...
 @note Names and their associated images are not guaranteed to exist across
    ...
    convenience method like `+[MZGLStyle outdoorsStyleURLWithVersion:]`,
    `MZGLMapView`’s “Style URL” inspectable in Interface Builder, or a manually
...
/**
 ...
 set the `iconImageName` property of an `MZGLSymbolStyleLayer` object to that
 name.
 ...
 */
@property (nonatomic, strong) MZGLLight *light;
...
/**
 An object whose contents are represented by an `MZGLStyle` object that you
 configure.
 */
...
/**
 ...
 @note The default styles provided by Mapbox contain sources and layers with
    ...
    `+[MZGLStyle outdoorsStyleURLWithVersion:]` or a manually constructed
     `NSURL`.
 */

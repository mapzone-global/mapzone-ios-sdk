...
/**
 An `MZGLPointCollection` object represents a shape consisting of one or more
 ...
 You can add point collections to the map by adding them to an `MZGLShapeSource`
 object. Configure the appearance of an `MZGLShapeSource`’s or
 `MZGLVectorTileSource`’s point collections collectively using an
 `MZGLCircleStyleLayer` or `MZGLSymbolStyleLayer` object. To access a point
 collection’s attributes, use an `MZGLPointCollectionFeature` object.

 You cannot add an `MZGLPointCollection` object directly to a map view as an
 annotation. However, you can create individual `MZGLPointAnnotation` objects
 from the `coordinates` array and add those annotation objects to the map view
 using the `-[MZGLMapView addAnnotations:]` method.

 A point collection is known as a
 <a href="https://tools.ietf.org/html/rfc7946#section-3.1.3">MultiPoint</a>
 geometry in GeoJSON. Do not confuse `MZGLPointCollection` with `MZGLMultiPoint`,
 the abstract superclass of `MZGLPolyline` and `MZGLPolygon`.
 */
MGL_EXPORT
@interface MZGLPointCollection : MZGLShape <MZGLOverlay>

/**
 Creates and returns a `MZGLPointCollection` object from the specified set of
 coordinates.

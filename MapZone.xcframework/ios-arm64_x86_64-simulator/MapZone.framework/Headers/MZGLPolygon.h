...
/**
 An `MZGLPolygon` object represents a closed shape consisting of four or more
 ...
 You can add polygon shapes to the map by adding them to an `MZGLShapeSource`
 object. Configure the appearance of an `MZGLShapeSource`’s or
 `MZGLVectorTileSource`’s polygons collectively using an `MZGLFillStyleLayer` or
 `MZGLSymbolStyleLayer` object. To access a polygon’s attributes, use an
 `MZGLPolygonFeature` object.
 ...
 `-[MZGLMapViewDelegate mapView:fillColorForPolygonAnnotation:]`.
 ...
 `MZGLPolygon` objects with the `+polygonWithCoordinates:count:interiorPolygons:`
 ...
 an `MGLMultiPolygon` or `MZGLShapeCollection` object.
 ...
 `MZGLPolygon` object from an array of coordinates.
 */
MGL_EXPORT
@interface MZGLPolygon : MZGLMultiPoint <MZGLOverlay>
 ...
 */
@property (nonatomic, nullable, readonly) NSArray<MZGLPolygon *> *interiorPolygons;

/**
 Creates and returns an `MZGLPolygon` object from the specified set of
 coordinates.
...
/**
 ...
 @param coords The array of coordinates defining the shape. The data in this
    array is copied to the new object.
 @param count The number of items in the `coords` array.
 @param interiorPolygons An array of `MZGLPolygon` objects that define regions
    excluded from the overall shape. If this array is `nil` or empty, the shape
    is considered to have no interior polygons.
 @return A new polygon object.
 */
+ (instancetype)polygonWithCoordinates:(const CLLocationCoordinate2D *)coords count:(NSUInteger)count interiorPolygons:(nullable NSArray<MZGLPolygon *> *)interiorPolygons;
...
/**
 ...
 atoll: the inner island would be one `MZGLPolygon` object, while the surrounding
 ...
 `MZGLVectorTileSource`’s multipolygons collectively using an `MZGLFillStyleLayer`
 or `MZGLSymbolStyleLayer` object.
 ...
 */
MGL_EXPORT
@interface MGLMultiPolygon : MZGLShape <MZGLOverlay>

/**
 An array of polygons forming the multipolygon.
 */
@property (nonatomic, copy, readonly) NSArray<MZGLPolygon *> *polygons;
 ...
 */
+ (instancetype)multiPolygonWithPolygons:(NSArray<MZGLPolygon *> *)polygons;

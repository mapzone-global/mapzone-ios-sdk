...
/**
 `MZGLVectorTileSource` is a map content source that supplies tiled vector data
 ...
 A vector tile source is added to an `MZGLStyle` object along with one or more
 `MZGLVectorStyleLayer` objects. A vector style layer defines the appearance of
 any content supplied by the vector tile source.
 
 `MZGLVectorTileSource` is optimized for data sets that are too large to fit
 ...
 <a href="http://geojson.org/">GeoJSON</a> data, use the `MZGLShapeSource`
 class. For tiled data that changes dynamically, the `MZGLComputedShapeSource`
 ...
 `MZGLVectorTileSource` object that you can use to initialize new style layers.
 ...
 ```swift
 let source = MZGLVectorTileSource(identifier: "pois", tileURLTemplates: ["https://example.com/vector-tiles/{z}/{x}/{y}.mvt"], options: [
     .minimumZoomLevel: 9,
     .maximumZoomLevel: 16,
     .attributionInfos: [
         MZGLAttributionInfo(title: NSAttributedString(string: "© Mapbox"), url: URL(string: "https://mapbox.com"))
     ]
 ])
 ...
 an `MZGLVectorTileSource` object.
 */
MGL_EXPORT
@interface MZGLVectorTileSource : MZGLTileSource
...
/**
 ...
 @param options A dictionary containing configuration options. See
    `MZGLTileSourceOption` for available keys and values. Pass in `nil` to use
    the default values.
 @return An initialized tile source.
 */
- (instancetype)initWithIdentifier:(NSString *)identifier tileURLTemplates:(NSArray<NSString *> *)tileURLTemplates options:(nullable NSDictionary<MZGLTileSourceOption, id> *)options NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 source loads a feature if the source is added to an `MZGLMapView`’s style; that
 ...
 */
- (NSArray<id <MZGLFeature>> *)featuresInSourceLayersWithIdentifiers:(NSSet<NSString *> *)sourceLayerIdentifiers predicate:(nullable NSPredicate *)predicate NS_SWIFT_NAME(features(sourceLayerIdentifiers:predicate:));

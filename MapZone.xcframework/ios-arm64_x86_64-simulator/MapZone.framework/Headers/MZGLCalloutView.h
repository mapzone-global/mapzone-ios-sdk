...
@protocol MZGLCalloutViewDelegate;
...
/**
 ...
 To receive updates from an object that conforms to the `MZGLCalloutView` protocol,
 use the optional methods available in the `MZGLCalloutViewDelegate` protocol.
 ...
 `MZGLCalloutView`.
 */
@protocol MZGLCalloutView <NSObject>

/**
 An object conforming to the `MZGLAnnotation` protocol whose details this callout
 view displays.
 */
@property (nonatomic, strong) id <MZGLAnnotation> representedObject;
...
/**
 An object conforming to the `MZGLCalloutViewDelegate` method that receives
 messages related to the callout view’s interactive subviews.
 */
@property (nonatomic, weak) id<MZGLCalloutViewDelegate> delegate;
...
/**
 ...
 `-[MZGLCalloutView presentCalloutFromRect:inView:constrainedToRect:animated:]`
...
/**
 The `MZGLCalloutViewDelegate` protocol defines a set of optional methods that
 you can use to receive messages from an object that conforms to the
 `MZGLCalloutView` protocol. The callout view uses these methods to inform the
 delegate that the user has interacted with the the callout view.
 */
@protocol MZGLCalloutViewDelegate <NSObject>
 ...
 */
- (BOOL)calloutViewShouldHighlight:(UIView<MZGLCalloutView> *)calloutView;

/**
 Tells the delegate that the callout view has been tapped.
 */
- (void)calloutViewTapped:(UIView<MZGLCalloutView> *)calloutView;

/**
 Called before the callout view appears on screen, or before the appearance
 animation will start.
 */
- (void)calloutViewWillAppear:(UIView<MZGLCalloutView> *)calloutView;

/**
 Called after the callout view appears on screen, or after the appearance
 animation is complete.
 */
- (void)calloutViewDidAppear:(UIView<MZGLCalloutView> *)calloutView;

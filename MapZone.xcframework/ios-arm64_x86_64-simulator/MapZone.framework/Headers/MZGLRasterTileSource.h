...
/**
 ...
 This option is only applicable to `MZGLRasterTileSource` objects; it is ignored
 when initializing `MZGLVectorTileSource` objects.
 */
FOUNDATION_EXTERN MGL_EXPORT const MZGLTileSourceOption MZGLTileSourceOptionTileSize;

/**
 `MZGLRasterTileSource` is a map content source that supplies raster image tiles
 ...
 A raster tile source is added to an `MZGLStyle` object along with one or more
 ...
 `MZGLRasterTileSource` object that you can use to initialize new style layers. You
 ...
 ```swift
 let source = MZGLRasterTileSource(identifier: "clouds", tileURLTemplates: ["https://example.com/raster-tiles/{z}/{x}/{y}.png"], options: [
     .minimumZoomLevel: 9,
     .maximumZoomLevel: 16,
     .tileSize: 512,
     .attributionInfos: [
 ...
 Add raster imagery</a> example to learn how to add a `MZGLRasterStyleLayer`
 to your map using an `MZGLRasterTileSource`.
 */
MGL_EXPORT
@interface MZGLRasterTileSource : MZGLTileSource
...
/**
 ...
 `MZGLTileSourceOptionTileSize` documentation for more information about tile
...
/**
 ...
 @param tileSize The width and height (measured in points) of each tiled image
    in the raster tile source. See the `MZGLTileSourceOptionTileSize`

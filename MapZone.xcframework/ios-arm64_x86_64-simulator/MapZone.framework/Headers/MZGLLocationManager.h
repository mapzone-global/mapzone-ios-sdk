...
@protocol MZGLLocationManagerDelegate;

/**
 The `MZGLLocationManager` protocol defines a set of methods that a class must
 implement in order to serve as the location manager of an `MZGLMapView`. A location
 ...
 To receive location updates from an object that conforms to the `MZGLLocationManager`
 protocol, use the optional methods available in the `MZGLLocationManagerDelegate` protocol.
 */
@protocol MZGLLocationManager <NSObject>
...
/**
 ...
 The default value of this property is `kCLDistanceFilterNone` when `MZGLMapView` uses its
...
/**
 ...
 The default value is `kCLLocationAccuracyBest` when `MZGLMapView` uses its
 default location manager.
...
/**
 ...
 `MZGLLocationManager.desiredAccuracy` property to any value. If the value is `CLAccuracyAuthorizationReducedAccuracy`,
 setting `MZGLLocationManager.desiredAccuracy` to a value other than` kCLLocationAccuracyReduced` has no effect on
...
/**
 ...
 The default value is `CLActivityTypeOther` when `MZGLMapView` uses its
 default location manager.
...
/**
 ...
 Do not set the location manager’s delegate yourself. `MZGLMapView` sets this property
 after the location manager becomes `MZGLMapView`’s location manager.
 */
@property (nonatomic, weak) id<MZGLLocationManagerDelegate> delegate;
...
/**
 The `MZGLLocationManagerDelegate` protocol defines a set of methods that respond
 to location updates from an `MZGLLocationManager` object that is serving as the
 location manager of an `MZGLMapView`.
 */
@protocol MZGLLocationManagerDelegate <NSObject>
...
/**
 ...
 contains multiple `CLLocation` objects when `MZGLMapView` uses  its
 default location manager.
 */
- (void)locationManager:(id<MZGLLocationManager>)manager
 ...
 */
- (BOOL)locationManagerShouldDisplayHeadingCalibration:(id<MZGLLocationManager>)manager;
 ...
 */
- (void)locationManagerDidChangeAuthorization:(id<MZGLLocationManager>)manager;
#endif       

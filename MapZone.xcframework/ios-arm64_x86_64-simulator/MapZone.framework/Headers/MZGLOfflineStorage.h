...
@class MZGLOfflinePack;
@protocol MZGLOfflineRegion;
@protocol MZGLOfflineStorageDelegate;

/**
 Posted by the shared `MZGLOfflineStorage` object when an `MZGLOfflinePack`
 ...
 The `object` is the `MZGLOfflinePack` object whose progress changed. The
 `userInfo` dictionary contains the pack’s current state in the
 `MZGLOfflinePackUserInfoKeyState` key and details about the pack’s current
 progress in the `MZGLOfflinePackUserInfoKeyProgress` key. You may also consult
 the `MZGLOfflinePack.state` and `MZGLOfflinePack.progress` properties, which
 ...
 */
FOUNDATION_EXTERN MGL_EXPORT const NSNotificationName MZGLOfflinePackProgressChangedNotification;

/**
 Posted by the shared `MZGLOfflineStorage` object whenever an `MZGLOfflinePack`
 ...
 The `object` is the `MZGLOfflinePack` object that encountered the error. The
 `userInfo` dictionary contains the error object in the
 `MZGLOfflinePackUserInfoKeyError` key.
 */
FOUNDATION_EXTERN MGL_EXPORT const NSNotificationName MZGLOfflinePackErrorNotification;

/**
 Posted by the shared `MZGLOfflineStorage` object when the maximum number of
 Mapbox-hosted tiles has been downloaded and stored on the current device.

 The `object` is the `MZGLOfflinePack` object that reached the tile limit in the
 course of downloading. The `userInfo` dictionary contains the tile limit in the
 `MZGLOfflinePackUserInfoKeyMaximumCount` key.

 Once this limit is reached, no instance of `MZGLOfflinePack` can download
 additional tiles from Mapbox APIs until already downloaded tiles are removed by
 calling the `-[MZGLOfflineStorage removePack:withCompletionHandler:]` method.
 */
FOUNDATION_EXTERN MGL_EXPORT const NSNotificationName MZGLOfflinePackMaximumMapboxTilesReachedNotification;

/**
 A key in the `userInfo` property of a notification posted by `MZGLOfflinePack`.
 */
typedef NSString *MZGLOfflinePackUserInfoKey NS_EXTENSIBLE_STRING_ENUM;
...
/**
 ...
 `MZGLOfflinePackProgressChangedNotification` notification. Call `-integerValue`
 on the object to receive the `MZGLOfflinePackState`-typed state.
 */
FOUNDATION_EXTERN MGL_EXPORT const MZGLOfflinePackUserInfoKey MZGLOfflinePackUserInfoKeyState;
...
/**
 ...
 `MZGLOfflinePackProgressChangedNotification` notification. Call
 `-MZGLOfflinePackProgressValue` on the object to receive the
 `MZGLOfflinePackProgress`-typed progress.
 */
FOUNDATION_EXTERN MGL_EXPORT const MZGLOfflinePackUserInfoKey MZGLOfflinePackUserInfoKeyProgress;
...
/**
 ...
 an `MZGLOfflinePackErrorNotification` notification. The error’s domain is
 `MGLErrorDomain`. See `MGLErrorCode` for possible error codes.
 */
FOUNDATION_EXTERN MGL_EXPORT const MZGLOfflinePackUserInfoKey MZGLOfflinePackUserInfoKeyError;
...
/**
 ...
 `MZGLOfflinePackMaximumMapboxTilesReachedNotification` notification. Call
 `-unsignedLongLongValue` on the object to receive the `uint64_t`-typed tile
 limit.
 */
FOUNDATION_EXTERN MGL_EXPORT const MZGLOfflinePackUserInfoKey MZGLOfflinePackUserInfoKeyMaximumCount;
 ...
 */
typedef void (^MZGLOfflinePackAdditionCompletionHandler)(MZGLOfflinePack * _Nullable pack, NSError * _Nullable error);
 ...
 */
typedef void (^MZGLOfflinePackRemovalCompletionHandler)(NSError * _Nullable error);
 ...
 */
typedef void (^MGLBatchedOfflinePackAdditionCompletionHandler)(NSURL *fileURL, NSArray<MZGLOfflinePack *> * _Nullable packs, NSError * _Nullable error);
...
/**
 MZGLOfflineStorage implements a singleton (shared object) that manages offline
 ...
 */
MGL_EXPORT
@interface MZGLOfflineStorage : NSObject

/**
 Returns the shared offline storage object.
 */
@property (class, nonatomic, readonly) MZGLOfflineStorage *sharedOfflineStorage;
...
/**
 ...
 or endpoints.
 */
@property(nonatomic, weak, nullable) IBOutlet id<MZGLOfflineStorageDelegate> delegate;
...
/**
 ...
 [`MZGLOfflineStorageDatabasePath`](../infoplist-keys.html#mglofflinestoragedatabasepath)
 key in Info.plist.
 */
 ...
 */
@property (nonatomic, strong, readonly, nullable) NSArray<MZGLOfflinePack *> *packs;
...
/**
 ...
 The pack has an initial state of `MZGLOfflinePackStateInactive`. To begin
 downloading resources, call `-[MZGLOfflinePack resume]` on the pack from within
 the completion handler. To monitor download progress, add an observer for
 `MZGLOfflinePackProgressChangedNotification`s about that pack.
 ...
 */
- (void)addPackForRegion:(id <MZGLOfflineRegion>)region withContext:(NSData *)context completionHandler:(nullable MZGLOfflinePackAdditionCompletionHandler)completion;
 ...
 */
- (void)removePack:(MZGLOfflinePack *)pack withCompletionHandler:(nullable MZGLOfflinePackRemovalCompletionHandler)completion;
...
- (void)invalidatePack:(MZGLOfflinePack *)pack withCompletionHandler:(void (^)(NSError * _Nullable))completion;
/**
 Forcibly, asynchronously reloads the `packs` property. At some point after this
 method is called, the pointer values of the `MZGLOfflinePack` objects in the
...
/**
 ...
 `MZGLOfflinePackMaximumMapboxTilesReachedNotification` is posted for every
...
/**
 ...
 as part of an offline pack or due to caching during normal use of `MZGLMapView`.
 */
...
/**
 The `MZGLOfflineStorageDelegate` protocol defines methods that a delegate of an
 `MZGLOfflineStorage` object can optionally implement to transform various types
 of URLs before downloading them via the internet.
 */
@protocol MZGLOfflineStorageDelegate <NSObject>
 ...
 */
- (NSURL *)offlineStorage:(MZGLOfflineStorage *)storage
